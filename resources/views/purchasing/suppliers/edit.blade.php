...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-black text-gray-900 dark:text-white">تعديل: {{ $supplier->name }}</h1>
    </div>
        ...
        <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div><label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">كود المورد <span class="text-red-500">*</span></label>
                    <input type="text" name="code" value="{{ old('code', $supplier->code) }}" required class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white"></div>
                <div><label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">اسم المورد <span class="text-red-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $supplier->name) }}" required class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white"></div>
                <div><label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">الهاتف</label>
                    <input type="text" name="phone" value="{{ old('phone', $supplier->phone) }}" class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white"></div>
                <div><label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">الرصيد الافتتاحي</label>
                    <input type="number" step="0.01" name="opening_balance" value="{{ old('opening_balance', $supplier->opening_balance) }}" class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white"></div>
            </div>
            <div><label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">العنوان</label>
                <textarea name="address" rows="2" class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white">{{ old('address', $supplier->address) }}</textarea></div>
            <div><label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">نوع الرصيد</label>
                <select name="balance_type" class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:bg-dark-card dark:text-white">
                    <option value="credit" {{ old('balance_type', $supplier->balance_type) === 'credit' ? 'selected' : '' }}>دائن</option>
                    <option value="debit" {{ old('balance_type', $supplier->balance_type) === 'debit' ? 'selected' : '' }}>مدين</option>
                </select></div>
