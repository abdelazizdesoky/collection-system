...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-black text-gray-900 dark:text-white">فواتير الشراء</h1>
            <p class="text-sm text-gray-500 mt-1">إدارة فواتير المشتريات</p>
        </div>
        <a href="{{ route('purchase-invoices.create') }}" class="bg-gradient-to-l from-emerald-600 to-teal-600 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-emerald-500/30 hover:shadow-xl transition-all flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-dark-card rounded-3xl shadow-xl border border-gray-100 dark:border-dark-border p-6">
        <form action="{{ route('purchase-invoices.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="بحث برقم الفاتورة أو اسم المورد..." class="px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 dark:bg-dark-tableheader">
                        ...
                        <th class="text-right px-4 py-4 font-bold text-gray-700 dark:text-gray-300">المورد</th>
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-dark-border">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-dark-bg/30 transition-colors">
                        ...
                        <td class="px-4 py-4 font-bold text-gray-900 dark:text-white">{{ $inv->supplier->name ?? '-' }}</td>
                        ...
                        <td class="px-4 py-4 text-center">
                            <a href="{{ route('purchase-invoices.show', $inv) }}" class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 p-2 rounded-xl hover:bg-blue-200 transition-colors inline-block">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="10" class="px-6 py-12 text-center text-gray-400">لا توجد فواتير شراء بعد</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
