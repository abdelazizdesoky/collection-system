...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-black text-gray-900 dark:text-white">إضافة وحدة جديدة</h1>
    </div>
    <div class="bg-white dark:bg-dark-card rounded-3xl shadow-xl border border-gray-100 dark:border-dark-border p-8">
        <form action="{{ route('units.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="مثال: كيلوجرام" class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
                @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">الرمز</label>
                <input type="text" name="symbol" value="{{ old('symbol') }}" placeholder="مثال: كجم" class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
            </div>
