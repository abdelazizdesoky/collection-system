...
@section('title', 'خطط الزيارات - بوابة المندوب')
...
<div class="max-w-4xl mx-auto text-right" dir="rtl">
    ...
    <div class="bg-white dark:bg-dark-card rounded-2xl shadow-lg p-6 mb-8 border border-gray-100 dark:border-dark-border">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-gray-500 dark:text-gray-400">خطط الزيارات الخاصة بك اليوم بتاريخ {{ today()->format('Y/m/d') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="flex gap-4 mb-6">
        <a href="{{ route('collector.dashboard') }}" class="flex-1 text-center py-3 px-4 bg-white dark:bg-dark-card rounded-xl font-bold text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all border border-gray-200 dark:border-dark-border">
            <svg class="w-5 h-5 inline-block ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
            </svg>
            التحصيل
        </a>
        <a href="{{ route('collector.visits') }}" class="flex-1 text-center py-3 px-4 bg-gradient-to-r from-purple-500 to-indigo-500 rounded-xl font-bold text-white shadow-lg shadow-purple-500/20">
            <svg class="w-5 h-5 inline-block ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
            </svg>
            الزيارات
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            ...
                            <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 text-center border border-green-100 dark:border-slate-700">
                                <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $plan->visited_count }}</div>
                                <div class="text-xs text-green-500 dark:text-green-300">تمت الزيارة</div>
                            </div>
                            <div class="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 text-center border border-red-100 dark:border-slate-700">
                                <div class="text-2xl font-bold text-red-600 dark:text-red-400">{{ $plan->skipped_count }}</div>
                                <div class="text-xs text-red-500 dark:text-red-300">تم التخطي</div>
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-3">
                                <div class="bg-gradient-to-r from-purple-500 to-indigo-500 h-3 rounded-full transition-all" 
                                     style="width: {{ $plan->progress_percentage }}%"></div>
                            </div>
                        </div>

                        <!-- Enter Button -->
                        <a href="{{ route('collector.visit-plan', $plan) }}" 
                           class="block w-full bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white text-center py-4 rounded-xl font-bold text-lg transition-all transform hover:scale-[1.02]">
    ...
    @else
        ...
        <div class="bg-white dark:bg-dark-card rounded-2xl shadow-lg p-12 text-center text-right border border-gray-100 dark:border-dark-border" dir="rtl">
            ...
            <h2 class="text-2xl font-bold text-gray-700 dark:text-gray-200 mb-2">لا توجد خطط زيارات لليوم</h2>
            <p class="text-gray-500 dark:text-gray-400">لم يتم تعيين أي خطط زيارات لك اليوم. يرجى التواصل مع الإدارة.</p>
        </div>
    @endif
</div>
@endsection
