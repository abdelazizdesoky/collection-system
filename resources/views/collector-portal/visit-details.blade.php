                ...
                <div class="space-y-4">
                    ...
                    @elseif($visit->visit_type === 'issue')
                        <div class="bg-red-50 dark:bg-red-900/10 p-4 rounded-xl mb-4 flex justify-between items-center">
                            <div>
                                <p class="text-xs text-red-600 dark:text-red-400 mb-1">حالة المشكلة (عند الزيارة)</p>
                                <p class="text-lg font-black text-red-700 dark:text-red-300">{{ $visit->issue_status_label ?: ($visit->issue ? $visit->issue->status_label : 'غير محدد') }}</p>
                            </div>
                        ...
                        @if($visit->issue && $visit->issue_description != $visit->issue->description)
                        <div class="mb-4 p-4 bg-gray-50 dark:bg-slate-800/30 rounded-xl border-r-4 border-red-500">
                            <p class="text-xs font-bold text-red-600 dark:text-red-400 mb-1">المشكلة المرتبطة:</p>
                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ $visit->issue->description }}</p>
                        </div>
                        @endif

                        <div class="py-2">
                            <p class="text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">وصف المندوب / تحديث الحالة:</p>
                            <div class="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-xl border border-gray-100 dark:border-slate-700 text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{{ $visit->issue_description ?: ($visit->issue ? $visit->issue->description : 'لا توجد تفاصيل مسجلة') }}</div>
                        </div>
