    ...
    <div class="mb-8">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black dark:text-white tracking-tight">سجل التحصيلات</h1>
                    <p class="text-gray-500 dark:text-gray-400 font-medium">متابعة كافة عمليات التحصيل النقدي والبنكي</p>
                </div>
            </div>
        ...
        <div class="mt-8 bg-white dark:bg-dark-card p-4 rounded-2xl shadow-xl border border-gray-100 dark:border-dark-border">
            <form action="{{ route('collections.index') }}" method="GET" class="relative">
                ...
                <input type="text" 
                       ...
                       placeholder="بحث برقم الإيصال، اسم العميل، أو المحصل..." 
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                    @forelse ($collections as $collection)
                        <tr class="hover:bg-blue-50/30 dark:hover:bg-slate-700/30 transition-colors group">
                            <td class="px-6 py-5 whitespace-nowrap font-bold text-blue-600 dark:text-blue-400">#{{ $collection->receipt_no }}</td>
                            <td class="px-6 py-5 whitespace-nowrap">
                                <span class="font-bold dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $collection->customer->name }}</span>
                            </td>
                            <td class="px-6 py-5 whitespace-nowrap">
                                <span class="text-sm dark:text-gray-300">{{ $collection->collector->name }}</span>
                            </td>
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center justify-center gap-3">
                                    ...
                                    <form action="{{ route('collections.destroy', $collection) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white transition-all shadow-sm" onclick="return confirm('هل أنت متأكد من حذف هذا التحصيل نهائياً؟')" title="حذف">
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-20 text-center text-gray-400">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-xl font-medium">لم يتم تسجيل أي عمليات تحصيل حتى الآن.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
