...
@section('title', $showTrashed ? 'المندوبين المحذوفين' : 'مندوبين')
    ...
    <div class="mb-8">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black dark:text-white tracking-tight">{{ $showTrashed ? 'المندوبين المحذوفين' : 'المندوبين' }}</h1>
                    <p class="text-gray-500 dark:text-gray-400 font-medium">{{ $showTrashed ? 'إدارة المندوبين المحذوفين مؤقتاً' : 'إدارة فريق مندوبين الميداني وصلاحياتهم' }}</p>
                </div>
            </div>
        ...
        <div class="mt-6 flex gap-2">
            <a href="{{ route('collectors.index') }}" 
               class="px-6 py-3 rounded-xl font-bold transition-all {{ !$showTrashed ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-gray-100 dark:bg-dark-card text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-dark-border' }}">
                <span class="flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    الكل ({{ $activeCount }})
                </span>
            </a>
            <a href="{{ route('collectors.index', ['trashed' => '1']) }}" 
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-right border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                    @forelse ($collectors as $collector)
                        <tr class="hover:bg-blue-50/30 dark:hover:bg-slate-700/30 transition-colors group {{ $showTrashed ? 'opacity-75' : '' }}">
                            ...
                            <td class="px-6 py-5 whitespace-nowrap">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 rounded-full {{ $showTrashed ? 'bg-red-100 dark:bg-red-900/30' : 'bg-indigo-100 dark:bg-indigo-900/30' }} flex items-center justify-center {{ $showTrashed ? 'text-red-600 dark:text-red-400' : 'text-indigo-600 dark:text-indigo-400' }} font-bold group-hover:scale-110 transition-transform">
                                        {{ mb_substr($collector->name, 0, 1) }}
                                    </div>
                                    <span class="font-bold dark:text-white text-lg group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors {{ $showTrashed ? 'line-through' : '' }}">{{ $collector->name }}</span>
                                </div>
                            </td>
                            ...
                            @if($showTrashed)
                            <td class="px-6 py-5 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $collector->deleted_at->format('Y-m-d H:i') }}</td>
                            @endif
                            <td class="px-6 py-5">
                                <div class="flex items-center justify-center gap-3">
                                    @if($showTrashed)
                                        <!-- Restore Button -->
                                        <form action="{{ route('collectors.restore', $collector->id) }}" method="POST" class="inline">
                                            @csrf
                                        ...
                                        <form action="{{ route('collectors.forceDelete', $collector->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white transition-all shadow-sm" onclick="return confirm('هل أنت متأكد من حذف هذا المندوب نهائياً؟ لا يمكن التراجع عن هذا الإجراء!')" title="حذف نهائي">
                                    ...
                                    @else
                                        ...
                                        <form action="{{ route('collectors.destroy', $collector) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white transition-all shadow-sm" onclick="return confirm('هل أنت متأكد من حذف هذا المندوب؟')" title="حذف">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                            </button>
                                        </form>
                                    @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $showTrashed ? '6' : '5' }}" class="px-6 py-20 text-center text-gray-400">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-xl font-medium">{{ $showTrashed ? 'لا يوجد مندوبين محذوفين.' : 'لم يتم إضافة أي مندوبين في النظام حتى الآن.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
