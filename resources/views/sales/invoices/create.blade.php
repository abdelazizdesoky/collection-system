...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-black text-gray-900 dark:text-white">فاتورة بيع جديدة</h1>
    </div>
    <form action="{{ route('sale-invoices.store') }}" method="POST" id="invoice-form">
        @csrf
        <div class="bg-white dark:bg-dark-card rounded-3xl shadow-xl border border-gray-100 dark:border-dark-border p-8 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div><label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">العميل <span class="text-red-500">*</span></label>
                    <select name="customer_id" required class="w-full px-4 py-3 border border-gray-200 dark:border-dark-border rounded-2xl bg-transparent dark:bg-dark-card dark:text-white select2-search">
                        <option value="">-- اختر العميل --</option>
                        @foreach($customers as $c)<option value="{{ $c->id }}" {{ old('customer_id') == $c->id ? 'selected' : '' }}>{{ $c->name }} ({{ $c->code }})</option>@endforeach
                    </select></div>
                ...
                <div id="items-container" class="space-y-3">
                    <div class="item-row grid grid-cols-12 gap-3 items-end">
                        <div class="col-span-4"><label class="block text-xs font-bold text-gray-500 mb-1">المنتج</label>
                            <select name="items[0][product_id]" required class="w-full px-3 py-2 border border-gray-200 dark:border-dark-border rounded-xl bg-transparent dark:bg-dark-card dark:text-white text-sm" onchange="fillPrice(this, 0, 'selling')">
                        ...
                        <div class="col-span-1 flex items-end pb-1">
                            <button type="button" onclick="removeRow(this)" class="bg-red-100 text-red-700 p-2 rounded-xl hover:bg-red-200"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex gap-3 pt-4">
                <button type="submit" class="bg-gradient-to-l from-blue-600 to-indigo-600 text-white px-8 py-3 rounded-2xl font-bold shadow-lg transition-all">حفظ الفاتورة</button>
                <a href="{{ route('sale-invoices.index') }}" class="bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 px-8 py-3 rounded-2xl font-bold transition-all">إلغاء</a>
            </div>
        </div>
    </form>
</div>
...
function addRow() {
    ...
    const html = `<div class="item-row grid grid-cols-12 gap-3 items-end">
        <div class="col-span-4"><select name="items[${rowIndex}][product_id]" required class="w-full px-3 py-2 border border-gray-200 dark:border-dark-border rounded-xl bg-transparent dark:bg-dark-card dark:text-white text-sm" onchange="fillPrice(this, ${rowIndex}, 'selling')">
            <option value="">اختر</option>
        ...
        <div class="col-span-1"><span class="block py-2 text-sm font-bold dark:text-white" id="row_total_${rowIndex}">0.00</span></div>
...
function removeRow(btn) { const rows = document.querySelectorAll('.item-row'); if(rows.length > 1) btn.closest('.item-row').remove(); }
function fillPrice(select, idx, type) {
    const opt = select.options[select.selectedIndex];
    const price = type === 'selling' ? opt.dataset.sellingPrice : opt.dataset.purchasePrice;
...
function calcRow(idx) {
    ...
    if(el) el.textContent = ((qty * price) - disc).toFixed(2);
}
