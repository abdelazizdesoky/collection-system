...
@section('title', $showTrashed ? 'الشيكات المحذوفة' : 'إدارة الشيكات')
    ...
    <div class="mb-8">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black dark:text-white tracking-tight">{{ $showTrashed ? 'الشيكات المحذوفة' : 'إدارة الشيكات' }}</h1>
                    <p class="text-gray-500 dark:text-gray-400 font-medium">{{ $showTrashed ? 'إدارة الشيكات المحذوفة مؤقتاً' : 'متابعة الشيكات المعلقة، المندوبة، والمرفوضة' }}</p>
                </div>
            </div>
        ...
        <div class="mt-6 flex gap-2">
            <a href="{{ route('cheques.index') }}" 
               class="px-6 py-3 rounded-xl font-bold transition-all {{ !$showTrashed ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/30' : 'bg-gray-100 dark:bg-dark-card text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-dark-border' }}">
                <span class="flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    الكل ({{ $activeCount }})
                </span>
            </a>
            <a href="{{ route('cheques.index', ['trashed' => '1']) }}" 
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                    @forelse ($cheques as $cheque)
                        <tr class="hover:bg-blue-50/30 dark:hover:bg-slate-700/30 transition-colors group {{ $showTrashed ? 'opacity-75' : '' }}">
                            <td class="px-6 py-5 whitespace-nowrap font-black text-lg text-blue-600 dark:text-blue-400 {{ $showTrashed ? 'line-through' : '' }}">#{{ $cheque->cheque_no }}</td>
                            <td class="px-6 py-5 whitespace-nowrap">
                                <span class="font-bold dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $cheque->customer->name ?? 'عميل محذوف' }}</span>
                            </td>
                            <td class="px-6 py-5 whitespace-nowrap">
                            ...
                            @if($showTrashed)
                            <td class="px-6 py-5 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $cheque->deleted_at->format('Y-m-d H:i') }}</td>
                            @else
                            <td class="px-6 py-5 text-center">
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center justify-center gap-3">
                                    @if($showTrashed)
                                        <!-- Restore Button -->
                                        <form action="{{ route('cheques.restore', $cheque->id) }}" method="POST" class="inline">
                                            @csrf
                                        ...
                                        <form action="{{ route('cheques.forceDelete', $cheque->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white transition-all shadow-sm" onclick="return confirm('هل أنت متأكد من حذف هذا الشيك نهائياً؟ لا يمكن التراجع عن هذا الإجراء!')" title="حذف نهائي">
                                    ...
                                    @else
                                        ...
                                        <form action="{{ route('cheques.destroy', $cheque) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white transition-all shadow-sm" onclick="return confirm('هل أنت متأكد من حذف هذا الشيك؟')" title="حذف">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                            </button>
                                        </form>
                                    @endif
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-20 text-center text-gray-400">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-xl font-medium">{{ $showTrashed ? 'لا توجد شيكات محذوفة.' : 'لم يتم تسجيل أي شيكات حتى الآن.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
